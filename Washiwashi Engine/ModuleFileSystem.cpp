...
ModuleFileSystem::ModuleFileSystem(Application* app, bool start_enabled) : Module(app, start_enabled)
{
	...
	{
		WASHI_LOG("File System error while creating write dir: %s\n", PHYSFS_getLastError());
...
{
	WASHI_LOG("Freeing File System subsystem");
...
bool ModuleFileSystem::AddPath(const char* path_or_zip)
{
	...
	{
		WASHI_LOG("File System error while adding a path or zip: %s\n", PHYSFS_getLastError());
	}
	else
		ret = true;
	...
	if (fs_file != nullptr)
	{
		...
		if (size > 0)
		{
			*buffer = new char[size + 1];
			...
			if (readed != size)
			{
				WASHI_LOG("File System error while reading from file %s: %s\n", file, PHYSFS_getLastError());
				if (buffer != nullptr)
				{
					delete[] buffer;
					buffer = nullptr;
		...
		if (PHYSFS_close(fs_file) == 0)
			WASHI_LOG("File System error while closing file %s: %s\n", file, PHYSFS_getLastError());
	}
	else
		WASHI_LOG("File System error while opening file %s: %s\n", file, PHYSFS_getLastError());
...
bool ModuleFileSystem::DuplicateFile(const char* srcFile, const char* dstFile)
{
	...
	if (srcOpen && dstOpen)
	{
		WASHI_LOG("[success] File Duplicated Correctly");
		return true;
	}
	else
	{
		WASHI_LOG("[error] File could not be duplicated");
		return false;
	}
}
...
uint ModuleFileSystem::Save(const char* file, const void* buffer, unsigned int size, bool append) const
{
	...
	if (fs_file != nullptr)
	{
		...
		if (written != size)
		{
			WASHI_LOG("[error] File System error while writing to file %s: %s", file, PHYSFS_getLastError());
		...
		else
		{
			if (append == true)
			{
				WASHI_LOG("Added %u data to [%s%s]", size, GetWriteDir(), file);
				//App->ConsoleLog("Added %u data to [%s%s]", size, GetWriteDir(), file);
			}
			else if (overwrite == true)
			{
				WASHI_LOG("File [%s%s] overwritten with %u bytes", GetWriteDir(), file, size);
				//App->ConsoleLog("File [%s%s] overwritten with %u bytes", GetWriteDir(), file, size);
			}
			else
			{
				WASHI_LOG("New file created [%s%s] of %u bytes", GetWriteDir(), file, size);
		...
		if (PHYSFS_close(fs_file) == 0)
		{
			WASHI_LOG("[error] File System error while closing file %s: %s", file, PHYSFS_getLastError());
	...
	else
	{
		...
		WASHI_LOG("[error] File System error while opening file %s: %s", file, PHYSFS_getLastError());
	}
...
bool ModuleFileSystem::Remove(const char* file)
{
	...
	if (file != nullptr)
	{
		...
		{
			WASHI_LOG("File deleted: [%s]", file);
			ret = true;
		}
		else
			WASHI_LOG("File System error while trying to delete [%s]: %s", file, PHYSFS_getLastError());
	}
